...
/// StartEmailUpdateRequest: Request type for `Members.start_email_update`.
#[derive(Serialize, Deserialize, Debug, Clone, Default)]
pub struct StartEmailUpdateRequest {
    /// organization_id: Globally unique UUID that identifies a specific Organization. The `organization_id` is
    /// critical to perform operations on an Organization, so be sure to preserve this value. You may also use
    /// the organization_slug or organization_external_id here as a convenience.
    pub organization_id: String,
    ...
    ///
    pub locale: std::option::Option<StartEmailUpdateRequestLocale>,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct StartEmailUpdateResponse {
    /// request_id: Globally unique UUID that is returned with every API call. This value is important to log
    /// for debugging purposes; we may ask for this value to help identify a specific API call when helping you
    /// debug an issue.
    pub request_id: String,
